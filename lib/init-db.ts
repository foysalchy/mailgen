...
export async function initDatabase() {
  ...
  try {
    console.log('Ensuring database `mailserver` exists...');
    ...
    // 4. Virtual Users / Mailboxes Table (Compatible with Postfix / Dovecot VPS authentication)
    await connection.query(`
    ...
    // 6. Webmail Messages Store (Allows immediate Webmail functionality & indexing)
    await connection.query(`
    ...
    const [plans]: any = await connection.query('SELECT COUNT(*) as cnt FROM plans');
    if (plans[0].cnt === 0) {
      await connection.query(`
